...
public class LayerWidget extends ListWidget {
    ...
    protected void renderWidget(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        for (int i = 0; i < info.length; i++) {
            drawScrollableText(context, ClientData.minecraft.textRenderer, info[i], this.getX(), this.getY()+getHeight()+i*infoHeight, this.getX()+this.getWidth(), this.getY()+getHeight()+(i+1)*infoHeight, (this.active ? 16777215 : 10526880) | MathHelper.ceil(this.alpha * 255.0F) << 24);
        }
    }
