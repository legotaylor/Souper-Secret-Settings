...
void main() {
    ...
    if (texCoord.y < 0.25 || texCoord.y > 0.75) {
        ...
        d = d*d*(3.0 - 2.0*d);
        col = mix(mix(texture(InSampler, texCoord), texture(BaseSampler, texCoord), 0.5), vec4(BackgroundColor, 0), d*(FarOpacity-NearOpacity)+NearOpacity);
    } else if (texCoord.x < 0.5) {
        col = texture(InSampler, (texCoord*2)-vec2(0,0.5));
    } else {
